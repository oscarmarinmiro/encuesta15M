...
//
          self.scaleState = d3.scale.ordinal().domain(["inactive","active","disconnected"]).range(["#000",self.scaleCompany.range()[0],"#de363a"]);
//
...
//        console.log(dataIn);
        ...
        {
            var mon = d['monetaryValue'];

            if(nowDateString in self.dataTL[d['userId']])
            {
                mon+=(self.dataTL[d['userId']][nowDateString]['cis'])*4;
            }

            return self.scaleMonetaryHeight(mon)*factor;
        }
    }
    ...
    self.buildGeometry = function(nowDate,dataIn,dataInS)
    {
        ...
        for(i=0;i<linksData.length;i++)
        {
           ...
           var color = self.getLinkStateColor(d);
...
//                color = self.scaleState(nodeState);
    ...
    self.changeCamera = function(cameraPos)
    {
