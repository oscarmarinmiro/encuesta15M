...

{
    ...
    self.backColor[0]= "#000";
    self.backColor[1]= "#222";
    self.backColor[2]= "#000";
    ...
    self.iluminaNews = function(nodo,d,i)
    {
            ...
            if (!d.children)
            {
                if(node.classed("chosen"))
                {
                    ...
                    node.selectAll("circle").transition().duration(self.transTime).style("fill",function(d){return d.children? self.backColor : self.bubbleColorScale(d.parent.parent.data.name);});
                    self.selected-=1;
...
//                    console.log(self.arrSelected);
                ...
                else
                {
                    if(self.selected<2)
                    {
                        ...
                        node.selectAll("text").transition().duration(self.transTime).style("fill",function(d){return d.children? self.backColor : self.bubbleColorScale(d.parent.parent.data.name);});
                        self.selected+=1;
...
//        console.log("Primer render...");
        ...
        self.nodes = self.news.append("circle")
            .on("mouseover",function(d,i){
                // Si no tiene hijas, padre + abuelo
                if(!d.children)
                {
//                    console.log(d);
                    self.tooltip.style("opacity",1.0).html(self.nameCallback(d.data.name, d.value.toFixed(2), d.children, self.removeUnderscores(d.parent.parent.data.name)+"</br>"+self.removeUnderscores(d.parent.data.name)));
                }
                else
                {
...
//                    console.log(d);
//
//                    console.log("Profundidad:"+ d.depth);
                    ...
                    if(d.depth==2)
                    {
                        self.tooltip.style("opacity",1.0).html(self.categoryCallback(self.removeUnderscores(d.parent.data.name),self.removeUnderscores(d.data.name),d.value.toFixed(2)));
                    }
                    ...
                    if(d.depth==1)
                    {
                        if(d.name!="root")
                        {
                            self.tooltip.style("opacity",1.0).html(self.categoryCallback(self.removeUnderscores(d.data.name),"",d.value.toFixed(2)));
                        }
                    }
...
//            .attr("title", function(d){ return d.data.name!='root' ? self.nameCallback(d.data.name, d.value.toFixed(2), d.children) : "";})
            .style("fill",function(d){
                    if (d.children)
                    {
                        if(d.depth==0)
                        {
                            return self.backColor[0];
                        }
                        if(d.depth==1)
                        {
                            return self.backColor[1];

                        }
                        if(d.depth==2)
                        {
                            return self.backColor[2];
                        }
                    }
                    else
                    {
                        if(d.depth==2)
                        {
                            return self.bubbleColorScale(d.parent.data.name);
                        }
                        else
                        {
                            return self.bubbleColorScale(d.parent.parent.data.name);
                        }
                    }
                })
//            .style("stroke-width",function(d){ if (d.children && d.data.name!='root'){ return "1px"} else {return "0px";}})
            .style("stroke-width",function(d){ return (d.children && d.data.name!='root')?"0.8px":"0px";})
            ...
            .style("stroke",function(d){
                if (d.children)
                {
                    ...
                    if(d.depth==1)
                    {
                        return self.bubbleColorScale(d.data.name);
                    }
                }
                else
                {
