...
tdviz.viz.networkDiagramCommunitiesEvents= function (options)
{
...
//        console.log(d);
    ...
    self.getColorCompany = function(d)
    {
        if(self.strokeChecked)
        {
            return self.scaleCompany(d['disCompany']);
        }
        else
    ...
    self.getColorCommunities = function(d)
    {
        ...
        {
            return self.colorCommunities(d['communities'][0]);
        }
    }
    ...
    self.render = function(data,nowDate,dataIn,eventsTL,checked)
    {
        self.myLog("En el render....",3)

        self.data = data;

        self.strokeChecked = checked;
...
//            .style("stroke",function(d,i){ return self.putStrokeColor(i);})
//            .style("stroke-width",function(d,i){ return self.putStrokeWidth(i);})
//            .style("stroke-opacity",function(d,i){ return self.strokeChecked? 1.0:0.3;})
            ...
            .style("stroke",function(d,i){return self.getColorCompany(d);})
...
//            .style("fill",function (d,i) {return self.fillCompany(self.getNodeState(d,i,nowDateString),d);})
            .style("visibility", function(d,i){return self.getNodeState(d,i,nowDateString)=='inactive'? "hidden":"visible";})
            .style("stroke",function(d,i){return self.getColorCompany(d);})
            .style("stroke-opacity",function(d,i){ return self.strokeChecked? 1.0:1.0;})
            .style("stroke-width",function(d,i){ return self.strokeChecked? "0.2px":"0.07px";})
//            .style("stroke",function(d,i){ return self.putStrokeColor(i);})
//            .style("stroke-width",function(d,i){ return self.putStrokeWidth(i);})
//            .style("stroke-opacity",function(d,i){ return self.putStrokeOpacity(i);})
            .attr("cx",function (d,i){return self.scalePosX(d['posx']);})
            .attr("cy",function (d,i){return self.scalePosY(d['posy']);})
            .attr("r",function (d,i){return self.scaleNodeSelect(d,dataIn,eventsTL,nowDateString);});
