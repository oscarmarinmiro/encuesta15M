...
//        console.log($('#siteOne').text());
    ...
    self.displayCategory = function(category,subcategory,value)
    {
        var html="";
        ...
        if (subcategory!="")
        {
            html+="<span class='big'>"+category+"</span></b><br>";
            html+="<span class='medium'>"+subcategory+"</span></b><br>";
            html+=parseFloat(value).toFixed(0)+" total visitors";
        }
        else
        {
            html+="<span class='big'>"+category+"</span></b><br>";
...
//        console.log(window.innerWidth);
        ...
        self.circleChart = tdviz.viz.circlePackingVisitorsInterfaz(
            {
                ...
                'categoryCallback': self.displayCategory
            });
        ...
        d3.json(self.baseJUrl+self.DATA_FILE, function(circleData)
        {
            if(circleData!=null)
            {
                ...
                for(var categoryIdx in circleData.children)
                {
                    for(var subcategoryIdx in circleData.children[categoryIdx].children)
                    {
                        for(var siteIdx in circleData.children[categoryIdx].children[subcategoryIdx].children)
                        {
                            var siteName = circleData.children[categoryIdx].children[subcategoryIdx].children[siteIdx].name;
                            if(self.pushedSites[siteName]==null)
                            {
                                self.availableSites.push(siteName);
                                self.pushedSites[siteName]=true;
                            }
                        }
                    }
